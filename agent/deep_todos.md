...
- 2026-07-31：M5C restore 核心已完成嚴格 SQL dump manifest、依賴拓撲與 drop-and-recreate 不可變 plan、checksummed tar/gzip package、30 分鐘加密 preview、PG 交易式與 MySQL partial restore session，以及 target catalog；GitHub Actions run `30642817243` 的 quality、Docker、browser、PostgreSQL 9.6/17 與 MySQL 5.6/8.4 全綠。
- 2026-07-31：SQL restore data entry 以既有 tagged NDJSON、mapping/import plan 與方言 SQL builder在同一 restore session 中執行，保留 bigint/decimal/NULL、PG sequence 與 MySQL AUTO_INCREMENT；active restore cancel 會 abort、等待 rollback/close，再將 job 標為 cancelled。
- 2026-07-31：完成 SQL dump encrypted staging package writer、snapshot catalog 與 export orchestration，核心表 snapshot 已保留 PG9.6 serial、PG10+ identity、MySQL AUTO_INCREMENT/FK 及 MySQL8.0.16+ CHECK；runtime/HTTP 依 server-side handler 分派 SQL export/restore並安全映射 preview、cancel、stale、driver failure及 MySQL partial progress。
- 2026-07-31：本輪完整本機驗證為114個Vitest檔通過、4個integration檔略過，435 tests通過、12 cases略過；lint、strict typecheck、production build及Playwright 7 passed/2 responsive skipped全綠。M5C仍待進階物件catalog、Web SQL dump/restore工作台及四版本進階roundtrip。
- 2026-08-01：M5C 前端完成 SQL dump scope/compression/include-data 設定與 SQL restore target/mode/database-name confirmation/skip-unsupported preview；preview token只保存在 React state，App tests 25/25 通過。
...
- 2026-08-01：新增真實 PostgreSQL 17、Redis 7 與固定版本 MinIO HA integration。run `30674742847` 先因不存在的 2025 MinIO container manifest 停在前置；改用實際發布的固定 `RELEASE.2024-03-30T09-41-56Z` 後，run `30675032770` 執行到測試本體並抓到舊版 MinIO 的批次刪除 `Content-MD5` 相容差異。以單元 RED 將 S3 cleanup 改為逐物件 `DeleteObject`，保留失敗重試與 metadata 追蹤語意。
- 2026-08-01：GitHub Actions run `30675239845` 全綠；HA job 真實驗證兩個 PostgreSQL metadata handles 的 advisory-lock migration、跨實例 session/revision 即時撤銷、Redis Pub/Sub wake 與故障後 PostgreSQL fallback、PG worker lease 唯一 owner/逾時接手，以及兩個 MinIO clients 跨實例應用層加密 chunk put/list/read/delete。兩 active + 一 standby API 流量拓撲與 30 秒晉升仍待完成。
- 2026-08-01：完成 PostgreSQL 權威 HA instance lease、最多兩個 active、standby readiness 503、worker 隨角色啟停，以及三 API + HAProxy 部署設定。首次 run `30676648195` 證明角色選舉正常但健康 API 未序列化角色；以 API RED 補安全 `active|standby` 欄位後，run `30677010327` 的 quality、Docker/HA Compose、browser、四版本資料庫與 ha-integration 全綠。HA job 真實驗證兩 active、一 standby、跨 active session，以及關閉 active 後 standby 在 30 秒內晉升。
...
- 2026-08-01：文件紀錄提交觸發的 Performance run `30684292871` 在candidate setup時遇到HAProxy readiness短暫抖動：單次health 200後login收到503，尚未進入candidate steady量測。先以workflow與runner測試建立RED，再要求baseline/candidate各連續三次readiness成功，且只對setup階段的502/503/504做有限重試；warmup/steady量測仍不重試，保留真實錯誤率。
- 2026-08-01：修復後Performance run `30685102140`歷時26分13秒完整通過。相同AMD EPYC 7763、4 logical CPUs、16,766,423,040 bytes RAM、Linux與Node 24.18.0上，baseline `1db99de`為95.40 req/s、p95 176.16ms、TTFB 174.90ms、error 0、peak RSS 779,270,226 bytes、growth 3.17%；candidate `15037b1`為95.44 req/s、p95 175.78ms、TTFB 174.28ms、error 0、peak RSS 775,736,525 bytes、growth -3.73%，全部相對及絕對門檻通過。同一revision的CI `30685102106`與Security `30685102102`亦完整全綠。
