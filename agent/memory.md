...
- 首次 Performance run `30681680040` 在負載前失敗：validator錯把`baseUrl`當正整數，always cleanup亦因必要Compose環境只存在step scope而無法展開。新增直接RED後，validator只投影五個profile欄位，workflow將必要環境提升到job scope。
- 正式 Performance run `30683306738` 歷時26分29秒完整通過。baseline revision `bf24a75`為91.92 req/s、p95 182.77ms、TTFB 181.29ms、error 0、peak RSS 772,601,283 bytes、growth 3.77%；candidate `72df831`為92.17 req/s、p95 182.37ms、TTFB 181.07ms、error 0、peak RSS 783,191,900 bytes、growth 4.87%。runner為AMD EPYC 7763、4 logical CPUs、16,766,423,040 bytes RAM、Linux、Node 24.18.0，所有相對與絕對門檻通過。
- 本機audit先抓到`@fastify/static@8.3.0` high path traversal advisory，固定升至10.1.1後high/critical為0（剩1 moderate）。固定`@cyclonedx/cdxgen@12.8.2`實際產生CycloneDX 1.7 SBOM共417 components；固定Gitleaks v8.30.1掃完整tracked history，production container驗證nonroot/read-only/cap-drop/no-new-privileges。
- deterministic hostile-input corpus先以RED發現keyset decoder只驗base64url canonical、未驗JSON canonical；新增固定JSON序列化比對後，tar traversal/header mutation、游標、DDL fragment與native grant控制字元語料全部通過。Security run `30683306732`與完整CI run `30683306761`全綠。
- 嘗試pnpm 11原生SBOM期間重建Windows `better-sqlite3`，本機缺Visual Studio C++ toolchain且該版本無可用prebuild；已撤回pnpm升級並以`pnpm install --ignore-scripts`恢復JS工具鏈。本機targeted/typecheck/lint/SBOM/audit可執行，完整native SQLite回歸改由GitHub Docker/quality job完成並全綠，未將本機限制誤報為通過。
- Performance run `30684292871` 的baseline完成，但candidate setup在單次readiness成功後遭遇HAProxy 503，因此沒有candidate artifact或compare結果。依TDD新增setup transient gateway retry及兩個phase連續三次readiness契約；重試只用於登入/建置profiles/百萬列setup，不套用warmup或steady請求。
- 最終Performance run `30685102140`完整通過。baseline `1db99de`在AMD EPYC 7763、4 CPUs、16,766,423,040 bytes RAM、Node 24.18.0上為95.395 req/s、p95 176.160ms、TTFB 174.902ms、error 0、peak RSS 779,270,226 bytes、growth 3.17%；candidate `15037b1`為95.442 req/s、p95 175.779ms、TTFB 174.280ms、error 0、peak RSS 775,736,525 bytes、growth -3.73%。相對p95/TTFB/throughput及絕對error/RSS門檻全部通過；CI `30685102106`與Security `30685102102`也在同一revision完整通過。
